...
void LOG(int* p1, int* p2, int k, double* p3,double*p4,double*p5);
...
//リーチング順番選定
//リーチングは、各ポイントデータから最も近い黒点のx座標が小さい順で行う。x座標が等しい場合は、
//各ポイントデータから最も近い黒点のｙ座標を1000で割ったとき、その商が奇数のときはｙが小さい順、偶数の時は大きい順で行う。
//つまり商が奇数の時はy軸正の方向、偶数の時はy軸負の方向に進む。
...
void Sort(int* p1, int* p2, int* p3, int k, double* p4, double* p5, double* p6) {
	...
	for (i = 0; i < NUM; i++) {
		//printf("  %d  %d", p1[i], p2[i]);
		//printf("  %d  %d", &p1[i], &p2[i]);
		//p1[i]でi番目の値にアクセス == *(p1+i)
		//double* p = 配列先頭ポインタ、のときのみ*p, *(p+1)またはp[i]で関節参照可
		//double* p[i]では単にポインタの配列なので関節参照には*p[i]
		t1[i] = &p1[i];
		t2[i] = &p2[i];
		t3[i] = &p3[i];
		t4[i] = &p4[i];
			...
			if (j1 == 0) {//最初
				...
				if (x1 != x2) {//x繰り上がりあり
					...
					for (j2 = 0; j2 < b; j2++) {
						...
						printf("  %3.2f deg  %3.2f deg  %3.2f deg\n", p3[0], p4[0], p5[0]);
